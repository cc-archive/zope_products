...
class LicenseEngine(UniqueObject, SimpleItem):
	...
	# user interface dispatch, etc
        ...
        index_html = PageTemplateFile(os.path.join(pt_dir, 'engine_index_html.pt'), globals())
	__call__ = index_html 
	...
	def licenseCodeToAnswers(self, licenseCode, jurisdiction='', version=None):
	...
	def issue(self, request=None):
		...
		if request.has_key('license_code'):
		   jurisdiction = (('jurisdiction' in request.keys()) and (request['jurisdiction'])) or \
						  ...
						  ''
		   answers = self.licenseCodeToAnswers(request['license_code'], jurisdiction, version = request.form.get('version', None))
		
		elif request.has_key('pd') or request.has_key('publicdomain'):
		...
		""" % (jurisdiction, request['field_commercial'], request['field_derivatives'], ('version' in request.form and request['version'] or ''))
	...
	def html_comment(self, xml_doc):
		"""Strip the <html> tags from the html xml element returned by the
		web service."""
	...
	def getLicenseWorkRdf(self, license_url, title=None, creator=None, 
		...
		""" % (license_url, work_rdf, lrdf) #self.issue()['licenserdf']) 
	...
	def languageJurisdiction(self, lang):
